...
public class MatchServiceImpl implements MatchService {
	...
	public LeagueTableView getLeagueStanding(Tournament tournament) {
		...
		TournamentSummaryView tournamentSummaryView = buildTournamentSummary(tournament);

		return tournamentSummaryView.getLeagueTableView();
	}
		...
		allTeams.forEach(t -> {
			...
			if (!matchDayScheduledTeam.contains(t)) {
				...
				if (CollectionUtils.isNotEmpty(opponents) && CollectionUtils.isNotEmpty(perRoundOpponentList)) {
					...
					log.info(t + "'s match has cannot be scheduled for round " + (roundIndex + 1)
							+ ", because either teams opponent list is empty or all opponent match has been scheduler. opponents size "
							+ opponents.size() + ", round oppo size : " + perRoundOpponentList.size());
				}
			} else {
	...
	public List<TeamSummaryView> sortTeamForLeague(List<TeamSummaryView> teamList) {
		...
		Comparator<TeamSummaryView> compareByGoalDifference = Comparator.comparing(TeamSummaryView::getGoalDifference)
				.reversed();
		Comparator<TeamSummaryView> compareByGoalForwarded = Comparator.comparing(TeamSummaryView::getGoalScored)
				.reversed();

		Comparator<TeamSummaryView> compareTeam = compareByPoint.thenComparing(compareByGoalDifference)
				.thenComparing(compareByGoalForwarded);
	...
	public StatView sortTeamForLeagueStat(StatView statView) {
		...
		List<TeamStatView> tmpGoalConcededPerGameList = new ArrayList<>();
		...
		statView.getGoalsConcededPerGame().forEach(t -> tmpGoalConcededPerGameList.add(t));
		
		tmpStatView.setCleansheets(tmpCleanSheetList.stream()
				.sorted(Comparator.comparing(TeamStatView::getCleanSheet).reversed()).collect(Collectors.toList()));
		tmpStatView.setGoalPerMatch(tmpGoalPerMatchList.stream()
				.sorted(Comparator.comparing(TeamStatView::getGoalsPerMatch).reversed()).collect(Collectors.toList()));
		tmpStatView.setPointsPerGame(tmpPointsPerGameList.stream()
				.sorted(Comparator.comparing(TeamStatView::getPointsPerMatch).reversed()).collect(Collectors.toList()));
		tmpStatView.setTopConceded(tmpTopConceded.stream()
				.sorted(Comparator.comparing(TeamStatView::getGoalAlloted).reversed()).collect(Collectors.toList()));
		tmpStatView.setTopScorers(tmpTopScorersList.stream()
				.sorted(Comparator.comparing(TeamStatView::getGoalForwarded).reversed()).collect(Collectors.toList()));
		tmpStatView.setGoalsConcededPerGame(tmpGoalConcededPerGameList.stream()
				.sorted(Comparator.comparing(TeamStatView::getGoalConcededPerGame).reversed()).collect(Collectors.toList()));
	...
	public TournamentSummaryView buildTournamentSummary(Tournament tournament) {
		...
		List<TeamStatView> goalConcededPerGameList = new ArrayList<>();
		...
		for (Team t : allTeam) {
			...
			for (Match m : matchList) {
				if (m.getTeamHome().intValue() == t.getId().intValue()) {
					if (m.getTeamHomeScore() != null) {
						...
						totalPenalty += (m.getTeamHomePenaltyPoint() != null ? m.getTeamHomePenaltyPoint().intValue()
								: 0);
					...
					if (m.getTeamAwayScore() != null) {
						...
						totalPenalty += (m.getTeamAwayPenaltyPoint() != null ? m.getTeamAwayPenaltyPoint().intValue()
								: 0);
			...
			goalConcededPerMatch = round(goalConcededPerMatch,2);
			...
			teamStatView.setGoalConcededPerGame(goalConcededPerMatch);
			...
			goalConcededPerGameList.add(teamStatView);
		...
		tmpStatView.setGoalsConcededPerGame(goalConcededPerGameList);
